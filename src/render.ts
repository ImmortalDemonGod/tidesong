...
export function render(ctx: CanvasRenderingContext2D, w: WorldState, ui: UIState): void {
  ...
  if (ui.bossIntro && !ui.victoryHold) {
    ...
    ctx.fillRect(0, 118, cw, 108);
    ctx.strokeStyle = accent;
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(cw / 2 - 260, 124);
    ctx.lineTo(cw / 2 + 260, 124);
    ctx.moveTo(cw / 2 - 260, 220);
    ctx.lineTo(cw / 2 + 260, 220);
    ctx.stroke();
    ctx.textAlign = "center";
    ctx.fillStyle = accent;
    ctx.font = "700 44px system-ui";
    ctx.fillText(bi.title, cw / 2, 176);
    ctx.fillStyle = C.muted;
    ctx.font = "600 16px ui-monospace, monospace";
    ctx.fillText(bi.sub, cw / 2, 208);
    ctx.textAlign = "left";
    ctx.restore();
  }
